...
for ram in range(1,11):
    for test in range(2,9):
        ...
        for test_time in range(time):
            menory=random.sample(range(0,8),test) # choose 2~8 numbers between 0 to 7
            w=np.zeros([56,56])
            for t in menory:         
            ...
            for i in range(8):
                a=random.sample(range(0,56), ram) # 1~10 mistakes
                for t in range(ram):
            ...
            for i in menory:
                ...
                for f in menory:
                    ...
                    if thr_i==0:
                        corr_tt+=1
            corr_t[test_time]=corr_tt/test  # 清洗成功數量/測試資料數量
